...
export default function DmsShell({title,eyebrow,subtitle,active,actions,children}:{title:string;eyebrow:string;subtitle:string;active:string;actions?:ReactNode;children:ReactNode}){
 ...
 <header className="dms-topbar commercial-topbar"><Link href="/dms" className="menu-button">☰</Link><div className="global-search"><span>⌕</span><input placeholder="Search customers, machines, jobs, invoices, serial numbers..."/></div><div className="topbar-actions"><Link href="/dms/create" className="quick-add" title="Quick create">+</Link><Link href="/dms/workshop" title="Workshop diary">▣</Link><Link href="/dms/customers" title="CRM">◌</Link><Link href="/dms/management" className="notify" title="Alerts">♢<sup>3</sup></Link><Link href="/tools" title="Help">?</Link><div className="branch-user"><div className="branch-avatar">RG</div><div><b>Risborough Garden Machinery</b><span>{depot}</span></div><select value={depot} onChange={e=>setDepot(e.target.value)}><option>Main Branch</option><option>All Branches</option><option>Demo Branch 2</option></select></div></div></header>
